...
//				      Updated again to add an IR sensor so that
//				      a remote control can turn on and off the 
//				      fire alert LEDs.
//

const uint8_t TEMP_PIN = A0;
const uint8_t INTERRUPT_PIN_A = 2; // For push button.
const uint8_t INTERRUPT_PIN_B = 3; // For IR sensor.
...
volatile uint8_t turn_off = LOW;    // Interrupt flag for IR sensor.
...
// Called by push button interrupt when manual intervention of LEDs is done for
// fire danger.
void evacuate()
{
...
void setup()
{
  ...
  attachInterrupt(digitalPinToInterrupt(INTERRUPT_PIN_B), turnOff, FALLING);
